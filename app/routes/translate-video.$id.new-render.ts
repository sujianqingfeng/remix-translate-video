...
export async function action({ params }: ActionFunctionArgs) {
	...
	await new Promise((resolve, reject) => {
		const ffmpeg = spawn('ffmpeg', [
			...
			maybePlayVideoFile,
			'-vf',
			`subtitles='${escapedCombinedSrtPath}':force_style='FontName=Microsoft YaHei,FontSize=17,Alignment=2,BorderStyle=1,Outline=0.4,Shadow=0,MarginV=42,PrimaryColour=&H00FFFF,OutlineColour=&H404040'`,
			'-c:v',
			'libx264',
			'-preset',
			'slow',
