...
export async function action({ params, request }: ActionFunctionArgs) {
	...
	// 生成 SRT 字幕文件
	const srtContent = generateSRT(transcripts)
	...
	await new Promise((resolve, reject) => {
		const ffmpeg = spawn('ffmpeg', [
			...
			maybePlayVideoFile,
			'-vf',
			`subtitles=${srtPath}:force_style='FontSize=16\\,Alignment=2\\,BorderStyle=0\\,Outline=0\\,Shadow=0\\,MarginV=20\\,PrimaryColour=&HFFFFFF'`,
			'-c:v',
			'libx264',
			'-preset',
			'medium',
